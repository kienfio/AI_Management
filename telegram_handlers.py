...
async def sales_commission_type_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "commission_percent":
        ...
        # 检查销售金额是否存在
        if 'sales_amount' not in context.user_data:
            logger.error("销售金额不存在，可能是会话过期")
            await query.edit_message_text(
...
async def show_sales_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # 检查必要的键是否存在
        required_keys = ['sales_person', 'sales_amount', 'sales_client']
        for key in required_keys:
            if key not in context.user_data:
                logger.error(f"关键数据'{key}'不存在，可能是会话过期")
                message = "❌ <b>Session expired or invalid</b>\n\nPlease start the process again."
...
async def sales_save_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # 检查必要的键是否存在
        required_keys = ['sales_person', 'sales_amount', 'sales_client', 'sales_commission']
        for key in required_keys:
            if key not in context.user_data:
                logger.error(f"关键数据'{key}'不存在，可能是会话过期")
                await query.edit_message_text(
...
async def sales_invoice_pdf_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # ✅ 获取 Telegram 文件对象
        file = await context.bot.get_file(file_id)
        logger.info(f"[UPLOAD] 准备下载 Telegram 文件: file_id={file_id}, file_name={file_name}")
        ...
        if result:
            ...
            await update.message.reply_text("❌ 发票PDF上传失败（返回结果为空）")
