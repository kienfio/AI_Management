...
async def setting_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info("显示系统设置菜单，包含创建负责人选项")
...
async def setting_category_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info(f"准备创建 {category_name}")
    
    # 如果是负责人，检查工作表是否存在
    if category_data == "pic":
        try:
            sheets_manager = SheetsManager()
            logger.info("检查负责人工作表是否存在")
            # 尝试获取负责人列表，这会触发工作表检查
            sheets_manager.get_pics(active_only=False)
        except Exception as e:
            logger.error(f"检查负责人工作表时发生错误: {e}")
            # 不中断流程，继续创建
    ...
    if category == "agent":
        ...
        try:
            ...
            if category == "worker":
                ...
                logger.info(f"尝试添加工作人员: {setting_data}")
                ...
                logger.info(f"尝试添加负责人: {setting_data}")
                # 添加更详细的日志记录
                try:
                    result = sheets_manager.add_pic(setting_data)
                    logger.info(f"添加负责人结果: {result}")
                except Exception as pic_error:
                    logger.error(f"添加负责人时发生异常: {pic_error}")
                    await update.message.reply_text(f"❌ Failed to add Person in Charge: {str(pic_error)}")
                    return ConversationHandler.END
            ...
            if result:
                ...
                if category == "pic":
                    # 对于负责人，显示更详细的错误信息
                    await update.message.reply_text("❌ Failed to add Person in Charge. Please check the logs for details.")
                else:
...
async def sale_invoice_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        for pic in pics:
            ...
            if not name:
                # 尝试其他可能的字段名
                for field in ['name', 'Name', '名字', '名称']:
                    if field in pic and pic[field]:
                        name = pic[field]
                        logger.info(f"使用替代字段 '{field}' 获取名称: {name}")
                        break
            
            if name:
                keyboard.append([InlineKeyboardButton(f"👤 {name}", callback_data=f"pic_{name}")])
                logger.info(f"添加负责人按钮: {name}")
        ...
        logger.error(f"处理销售记录命令时发生错误: {e}")
        import traceback
